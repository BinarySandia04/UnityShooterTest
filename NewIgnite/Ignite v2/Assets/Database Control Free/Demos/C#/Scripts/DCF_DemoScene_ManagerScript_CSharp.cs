...
public class DCF_DemoScene_ManagerScript_CSharp : MonoBehaviour {
    ...
    IEnumerator LoginUser ()
    {
        ...
        if (response == "Success")
        {
            ...
            {
                PlayerPrefs.SetString("Account/Username", playerUsername);
                PlayerPrefs.SetString("Account/Password", playerPassword);
                PlayerPrefs.SetInt("Account/AutoLogIn", 1);
                PlayerPrefs.Save();
                Debug.Log("Succes");
            }
    ...
    public void LoggedIn_LogoutButtonPressed ()
    {
        ...
        playerUsername = "";
        playerPassword = "";
        UserAccountManagement.instance.LogOut();
        loginParent.gameObject.SetActive(true);
        loggedInParent.gameObject.SetActive(false);
    }
}
